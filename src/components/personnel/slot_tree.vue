<template>
  <div class="slot-tree">
    ...
    <el-tree ref="SlotMenuList" class="expand-tree"
        	...
        	<!-- 未编辑状态 -->
        		<span v-show="!node.isEdit">
	        		...
	        		<!-- 新增按钮 -->
	        			<el-button icon="el-icon-plus" circle :size="iconSize" type="success" v-if="node.level!==3"
                           @click="NodeAdd(node, data)"></el-button>
                <!-- 编辑按钮 -->
	        			<el-button icon="el-icon-edit" circle :size="iconSize" type="primary"
                           @click="NodeEdit(node, data)"></el-button>
                <!-- 删除按钮 -->
	        			<el-button icon="el-icon-delete" circle :size="iconSize" type="warning"
    ...
    <field-dialog title="操作节点" ref="dialog" @confirm="dialogConfirm">
      <form-container ref="inlineForm" :model="inlineForm">
        <field-input v-model="inlineForm.name" label="名称" width="10"
                     :rules="r(true).all(R.require)" prop="name"></field-input>
        <field-input v-model="inlineForm.location" label="地点" width="10"
                     :rules="r(true).all(R.require)" prop="location"></field-input>
      </form-container>
    </field-dialog>
  </div>
</template>
    ...
    methods: {
      ...
      NodeAdd(n, d) {//新增节点
        ...
        if (n.level >= 3) {
          ...
          this.inlineForm = {};
          this.inlineForm.organUnit = {id: d.id};
          switch (n.level + 1) {
            case 1:
              this.inlineForm.level = 'MUNICIPAL';
              break;
            case 2:
              this.inlineForm.level = 'DISTRICT';
              break;
            case 3:
              this.inlineForm.level = 'POLICE_STATION';
              break;
          }
      ...
      dialogConfirm() {
        this.$refs.inlineForm.gqlValidate(api.addOrganUnit,
          {organUnit: this.inlineForm}, (res) => {
            console.log(res);
            this.$message.success("添加成功！");
            this.$emit('treeChange', true);
